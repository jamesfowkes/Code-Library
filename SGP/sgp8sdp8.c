...
void sgp8 (double tsince, struct vector *pos, struct vector *vel, struct sgp_data *satdata)
{
	...
	double M=0, Ew=0, deltaEw=0, a=0, _sinf=0, _cosf=0, rii=0;
	...
	a = pow((xke / n), _tothrd);
	beta = sqrt(1.0 - sqr(e));
	_sinf = beta * sin(Ew) * (1.0 / (1.0 - e * cos(Ew)));
	_cosf = (cos(Ew) - e) * (1.0 / (1.0 - e * cos(Ew)));
	f = actan(_sinf, _cosf);
	sinu = _sinf * cos(omega) + _cosf * sin(omega);
	cosu = _cosf * cos(omega) - _sinf * sin(omega);
	rii = (a * (1.0 - sqr(e))) / (1.0 + (e * _cosf));
	...
	deltaI = temp * cosio;
	deltaU = sin(satdata->xincl / 2) * ((0.5 * CK2 * sqr(1.0 / (a * (1 - sqr(e))))) * (0.5 * (1.0 - 7.0 * sqr(theta)) * (2.0 * sinu * cosu)- 3.0 * (1.0 - 5.0 * sqr(theta)) * (f - M + e * _sinf)) - (0.25 * A30 * (1.0 / (a * (1.0 - sqr(e))))) * sinio * cosu * (2.0 + (e*_cosf))) - 0.5 * (0.25 * A30 * (1.0 / (a * (1 - sqr(e))))) * sqr(theta) * e * cos(omega) / cos(satdata->xincl/2);
	lambda = f + omega + xnode + (0.5 * CK2 * sqr(1.0 / (a * (1.0 - sqr(e))))) * (0.5 * (1.0 + 6.0 * cosio - 7.0 * sqr(theta)) * (2.0 * sinu * cosu) - 3.0 * ((1.0 - 5.0 * sqr(theta)) + 2.0 * cosio) * (f - M + e * _sinf )) + (0.25 * A30 * (1.0 / (a *(1 - sqr(e))))) * sinio * (cosio * e * cos(omega) / (1.0 + cosio) - (2.0 + (e*_cosf)) * cosu);
	...
	r = rii + deltaR;
	rdot = n * a * e * _sinf / beta + (-n * sqr(a / rii)) * (1.0 * CK2 * (1.0 / (a * (1.0 - sqr(e)))) * (1.0 - sqr(theta)) * (2.0 * sinu * cosu) + (0.25 * A30 * sinio) * cosu);
...
void sdp8 (double tsince, struct vector *pos, struct vector *vel, struct sgp_data *satdata)
{
	...
	double snlamb=0, cslamb=0;
        double snfg=0, csfg=0, sn2f2g=0, cs2f2g=0, e_cosf=0, g10=0, rm=0, aovr=0, g13=0, g14=0, sini2=0;
	int i;
	...
	di = diwc * cosi;
	sini2 = sin(0.5 * xinc);
	sni2du = sinio2 * (g3 * (0.5 * (1.0 - 7.0 * theta2) * sn2f2g - 3.0 * unm5th * g10) - g5 * sini * csfg * (2.0 + e_cosf)) - 0.5 * g5 * theta2 * axnm / cosio2;
	xlamb = fm + omgasm + xnodes + g3 * (0.5 * (1.0 + 6.0 * cosi - 7.0 * theta2) * sn2f2g - 3.0 * (unm5th + 2.0 * cosi) * g10) + g5 * sini * (cosi * axnm / (1.0 + cosi) - (2.0 + e_cosf) * csfg);
